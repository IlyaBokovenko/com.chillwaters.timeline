# Purpose #
Timeline was developed as a part of core tech in [Match2](https://www.gamasutra.com/blogs/JonathanBailey/20150227/237544/Match_Game_Mechanics.php) mobile game. It represents a "time aspect" of some modelled system. Structurally it's just a container for time-ordered activities, which produce each other, depend on each other and do some work. It allows to push activities to certain points of time and subscribe to activites by instance or by type from inside or outside of Timeline. Timeline can be evolved forward in time manually or automatically, triggering activities along the way, which triggers subscriptions to activities' start and finish markers, which can lead to pushing new activities to Timeline and so on. From design perspective it resembles a hybrid of scheduler and message bus. Usually Timeline complements "state" data model, changing it by activties as model time advances.
# Features #
...
* Deterministic. Timeline is based on long-precision integer arithmetics, so no rounding problems. Timeline internally uses stable sorted collections, so same input always produces same results.
